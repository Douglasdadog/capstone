...
export default function TrackingIssueEmailModal({ issue, open, onClose, onSent }: Props) {
  ...
  if (!open || !issue || typeof window === "undefined") return null;

  const canSend = Boolean(issue.contact_email?.trim()) && subject.trim().length > 0 && body.trim().length > 0;

  return createPortal(
    <div
      className="fixed inset-0 z-[9999] overflow-y-auto bg-slate-900/50 p-4 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div className="flex min-h-full items-center justify-center">
        ...
        <div
          ref={panelRef}
          className="relative z-10 my-6 w-full max-w-lg overflow-hidden rounded-2xl border-2 border-slate-200 bg-white shadow-2xl shadow-slate-900/20 ring-1 ring-slate-200/80"
        >
          ...
          <div className="max-h-[min(85vh,720px)] overflow-y-auto px-5 py-4">
